...
# WATCH_FOLDER = "D:\SIH\Target"
# QUARANTINE_FOLDER = "D:\SIH\Quarantine"
# DB_PATH = os.path.join(os.path.dirname(__file__), 'database', 'scan_results.db')
DB_PATH = '../antivirus.db'
...
class WatcherHandler(FileSystemEventHandler):
    def __init__(self, username, ip, password, quarantineFolder):
    ...
    def process_event(self, file_path):
        ...
        process_file(file_path, self.username, self.ip, self.password, self.quarantineFolder)
...
def process_file(file_path, username, ip, password, quarantineFolder, getdb):
    ...
    try:
        ...
        if scan_status == "infected":
            try:
                quarantine_path = os.path.join(quarantineFolder, os.path.basename(file_path))
...
def watch_directory(ip, username, password, watchFolder, quarantineFolder, getdb ):
    ...
    os.makedirs(quarantineFolder, exist_ok=True)
    ...
    event_handler = WatcherHandler(username, ip, password, quarantineFolder)
    observer = Observer()
    observer.schedule(event_handler, path=watchFolder, recursive=False)
    observer.start()

    print(f"[INFO] Watching directory: {watchFolder}")
